...
(function () {
   ...
   * 顶级图案的基础倍率（EYE 8→14、JACKPOT 10→25）；三连小图案保持官方值不动
   * （它们是「债务竞速」的日常现金流，动它会拖垮前期节奏）。
   * 改完请重跑 `node _test/rtp_probe.js 40 120` 核对 per_pattern_RTP 份额。 */
  CP.PATTERN_BASE = {
   ...
   *
   * ⚠ 设计意图：本作是「债务竞速」而非「赌场平衡」，基础 RTP 远大于 1 是刻意的。
   *   请勿用传统 par sheet 的 80%–105% RTP 目标来「对齐」本表数值——
   *   两套机制不同（本作是「图案倍率制」，不是「连线数 × 符号赔付」）。
   *
   * 参照：_research/parsheet13_采纳建议.md、_test/ev_fixture.js（回归夹具）
   * -------------------------------------------------- */

  /* ---------------- 免费旋转 / 连锁奖励的期望值口径 ----------------
   * 来源：_research/parsheet13_采纳建议.md §3 P1-1（par sheet 复算已证实）
   *
   *   B = Σ [ P(单次旋转触发 k 个连锁源) × 初始赠送次数(k) ]
   *   E = 初始次数 n / (1 − B)      ← 含「免费旋转内再触发」的总期望次数
   *
   * 为什么必须用它：免费旋转会自我再触发，凭感觉给次数会让期望次数发散
   * （B ≥ 1 时直接爆炸）。本项目所有赠送旋转都满足 B < 1，并额外加每回合硬上限
   * （CP.FREE_SPIN_ROUND_CAP，见 engine.js 的 E.grantFreeSpins）。
   * 期望值回归夹具见 `_test/ev_fixture.js` 第 ⑧ 组。
   * -------------------------------------------------- */
  ...
  CP.FREE_SPIN_ROUND_CAP = 60;            // 单回合「赠送旋转」硬上限（防连锁发散）
